...
void inorder(BSTPTR T,stack &s,int &count)
{
    BSTPTR L=T;
    while(s.top!=-1 || L!=NULL)
    {
        while(L!=NULL)
        {
            push(s,L);
...
void preorder(BSTPTR T,stack &s,int &count)
{
    push(s,T);
    count++;
    while(s.top!=-1)
...
int main(){
   ...
   // assignxy(S,i,j);
    ...
    i=0;
    j=0;
    preorder(S,s,i);cout<<endl;
    cout<<i<<endl;
    s.top=-1;
    inorder(S,s,j);cout<<endl;
    cout<<j;
